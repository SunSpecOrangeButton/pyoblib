...
from ..validator import validate_concept_value, _get_validator_method_name, _xbrli_boolean_item_type_validator, \
    _xbrli_string_item_type_validator, _xbrli_integer_item_type_validator
...
class TestValidator(unittest.TestCase):
    ...
    def test_validate_concept_value(self):
        ...
        self.assertEqual(1, len(validate_concept_value(concept, None)[1]))
        concept.nillable = True
        self.assertEqual(0, len(validate_concept_value(concept, None)[1]))

        concept.type_name = "xbrli:booleanItemType"
        self.assertEqual(1, len(validate_concept_value(concept, "Arf")[1]))
        self.assertEqual(1, len(validate_concept_value(concept, 52)[1]))
        self.assertEqual(0, len(validate_concept_value(concept, False)[1]))
        self.assertEqual(0, len(validate_concept_value(concept, True)[1]))
        self.assertEqual(0, len(validate_concept_value(concept, "False")[1]))
        self.assertEqual(0, len(validate_concept_value(concept, "True")[1]))
        self.assertEqual(0, len(validate_concept_value(concept, "false")[1]))
        self.assertEqual(0, len(validate_concept_value(concept, "true")[1]))
        ...
        self.assertEqual(0, len(validate_concept_value(concept, True)[1]))
        self.assertEqual(0, len(validate_concept_value(concept, 52)[1]))
        self.assertEqual(0, len(validate_concept_value(concept, -52)[1]))

        concept.type_name = "xbrli:stringItemType"
        self.assertEqual(0, len(validate_concept_value(concept, False)[1]))
        self.assertEqual(0, len(validate_concept_value(concept, 52)[1]))
        self.assertEqual(0, len(validate_concept_value(concept, "Arf")[1]))
        self.assertEqual(0, len(validate_concept_value(concept, "")[1]))

        concept.id = "SomeIdentifier"
        self.assertEqual(1, len(validate_concept_value(concept, "Arf")[1]))
        self.assertEqual(0, len(validate_concept_value(concept, identifier())[1]))
    ...
    def test_get_validator_method_name(self):
        ...
        method_name_result = _get_validator_method_name(type_name)
    ...
    def test_xbrli_boolean_item_type_validator(self):
        ...
        for v in values.keys():
            result = _xbrli_boolean_item_type_validator(v)
    ...
    def test_xbrli_string_item_type_validator(self):
        values = {'frog': ('frog', []), 'Frog': ('Frog', []),
                  True: ('True', []), 0.3: ('0.3', [])}
        for v in values.keys():
            result = _xbrli_string_item_type_validator(v)
            self.assertEqual(result[0], values[v][0])
            if values[v][1]:  # error messages are present
                self.assertTrue(result[1])
            else:
    ...
    def test_xbrli_integer_item_type_validator(self):
        ...
        for v in values.keys():
            result = _xbrli_integer_item_type_validator(v)
            self.assertEqual(result[0], values[v][0])
            if values[v][1]:  # error messages are present
                self.assertTrue(result[1])
            else:
