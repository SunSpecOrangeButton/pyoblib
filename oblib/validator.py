...
class Validator(object):
    ...
    def validate_concept_value(self, concept, value):
        ...
        enum = self._taxonomy.types.get_type_enum(concept.type_name)
        is_enum = enum is not None
        # Check data type and validator calling
        if type(concept.type_name).__name__ in ["str", "unicode"]:
            method_name = self._get_validator_method_name(concept.type_name)
            ...
            if found_method is not None:
                ...
                result = self._generic_enum_validator(value, concept, enum)
            else:
    ...
    def _generic_enum_validator(self, value, concept, enum):
        errors = []
